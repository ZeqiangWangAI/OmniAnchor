"""Native acceptance checks against an already-loaded backend; never loads weights.

``verify_native`` compares the selected-logit scorer with an independent full-logit
forward, optionally checks the shared-prefill shortcut and cross-modality state reuse.
Full logits are materialized only for a bounded text sequence, while both scoring paths
normalize in FP32 without changing model precision. ``omnianchor verify-model`` wraps
it for a study configuration on a real checkpoint.
"""
...
from .backends.hf import HFBackend, PreparedCandidate
from .backends.media import freeze_media
from .errors import BudgetExceeded, ResourceUnavailable, OmniAnchorError
from .io import write_json
from .types import Anchor, Bridge, Part, Sample
...
def verify_native(
    ...
    prefix_text = backend.render_prefix(frozen, bridge)
